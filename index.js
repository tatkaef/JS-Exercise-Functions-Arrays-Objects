...
*/
function temperatureCtoF(temCels) {
  let temFahr = temCels*9/5+32;
  return `${temCels}C equals ${temFahr.toFixed(0)}F `;
}
...
*/
function temperatureInF(temperature, simbol) {
  if (simbol==='F' || 'f') {return `${temperature}F`};
  if (simbol==='C' || 'c') { return `${temperatureCtoF(temperature)}`};
}
